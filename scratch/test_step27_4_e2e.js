...
async function runStep27_4_Tests() {
  console.log('====================================================');
  console.log('  STEP 27.4 AUTOMATED E2E TEST SUITE (CORRECTED FLOW)');
  console.log('====================================================');
  ...
  // --------------------------------------------------------------------------
  // TEST 1: Telegram APPROVE -> DB Token Generation & OPEN BUTTON Message Update
  // --------------------------------------------------------------------------
  const userA = `sup_step27_4_a_${Date.now()}`;
  const reqIdA = `REQ-${Date.now()}-STEP27_4`;
  ...
  const test1Pass = webhookDataA.ok && tokenRecA && tokenRecA.request_id === reqIdA && reqDataA.message.includes('Waiting for approval');
  ...
  const rawToken2 = `test-step27-4-token-${Date.now()}`;
  ...
  // --------------------------------------------------------------------------
  const userB = `sup_step27_4_b_${Date.now()}`;
  const reqIdB = `REQ-${Date.now()}-STEP27_4_REJECT`;
  ...
  console.log(`  FINAL STEP 27.4 E2E VERIFICATION: ${allPass ? 'ALL TESTS PASSED ✅' : 'SOME TESTS FAILED ❌'}`);
  console.log('====================================================');
}

runStep27_4_Tests().catch(err => console.error('E2E Test Suite Error:', err));
