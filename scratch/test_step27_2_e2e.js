...
async function runStep27_2_Tests() {
  console.log('====================================================');
  console.log('  STEP 27.2 AUTOMATED END-TO-END TEST SUITE');
  console.log('====================================================');
  ...
  // --------------------------------------------------------------------------
  // TEST 1: Telegram APPROVE -> App Link Token Generation & Telegram URL Button
  // --------------------------------------------------------------------------
  const userA = `sup_applink_a_${Date.now()}`;
  const reqIdA = `REQ-${Date.now()}-APPLINK`;
  ...
  // Generate a test token and insert its hash into DB to verify Edge Function logic
  const rawToken2 = `test-raw-token-${Date.now()}`;
  const hash2 = sha256Hex(rawToken2);
  const expires2 = new Date(Date.now() + 5 * 60 * 1000).toISOString();

  // Insert token for reqIdA
  ...
  const expiredTime = new Date(Date.now() - 10 * 60 * 1000).toISOString(); // Expired 10 min ago
  ...
  // --------------------------------------------------------------------------
  // TEST 5: Wrong Request ID & Wrong Token
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  const userB = `sup_applink_b_${Date.now()}`;
  ...
  console.log(`  FINAL STEP 27.2 E2E VERIFICATION: ${allPass ? 'ALL TESTS PASSED ✅' : 'SOME TESTS FAILED ❌'}`);
  console.log('====================================================');
}

runStep27_2_Tests().catch(err => console.error('E2E Test Suite Error:', err));
